...
Since we got the certificate let's authenticate with PKINIT to the domain. We use then [PKINITools](https://dirkjanm.io/ntlm-relaying-to-ad-certificate-services/) kit's gettgtpkinit.py to obtain our TGT:
...
As we authenticated with PKINIT, client account NT hash is encrypted in the PAC (Privilege Attribute Certificate). In order to read the PAC and have access to client's NT hash, the client must use another Kerberos extension called user to user (U2U). User to User is well explained in [this IETF draft](https://datatracker.ietf.org/doc/html/draft-ietf-cat-user2user-02). The idea here is to request a service ticket to ourself while adding our TGT as "additional tickets" during the KRB_TGS_REQ. We will be able to find out the PAC and the NT hash when we receive KDC response as it uses the session key of our TGT (that we know) to encrypt the PAC.
...
As we have CURSE-COMP$ NT hash, we can perform [silver ticket](https://en.hackndo.com/kerberos-silver-golden-tickets/?_gl=1*1xbhafq*_ga*MTQ1MTUwNzkwOS4xNjk1MDIyMzU5*_ga_DNP6NC70FV*MTY5OTMwOTI1OS4yLjAuMTY5OTMwOTI1OS4wLjAuMA..#silver-ticket) attack and impersonate domain administrator (RID 500) user on the machine. Usually to perform S4U attacks, we use impacket. To proceed, we have to find domain SID followed by asking a domain administrator ticket to the computer account we compromised. Recently netexec added [nxc4u feature](https://www.netexec.wiki/news/v1.1.0-nxc4u) giving us a huge shortcut to perform this attack!
...
Let's go back to our lab, with this time a privileged user (**Satoru Gojo**) which logs in to CURSE-COMP with interractive shell tool. Gojo uses a script which connects to WSUS server through **Windows Server Update Services Class Library** to look for updates:
...
- [Juan Pablo Barriga](https://twitter.com/porquesiempretu) for his advices, suggestions and help
- [Thomas](https://twitter.com/_zblurx) and friends for the [netexec](https://www.netexec.wiki/)
- [Will Schroeder](https://twitter.com/harmj0y) and [Lee Christensen](https://twitter.com/tifkin_) for the [Certified Pre-Owned](https://specterops.io/wp-content/uploads/sites/3/2022/06/Certified_Pre-Owned.pdf) article
